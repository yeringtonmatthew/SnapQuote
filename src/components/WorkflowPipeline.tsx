      ...
      <div className="grid grid-cols-4 divide-x divide-gray-100 dark:divide-gray-800">
        {segments.map((seg) => (
          <Link
            key={seg.label}
            href={seg.href}
            className={`flex flex-col items-center px-2 py-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 press-scale`}
          >
            <div className={`flex h-8 w-8 items-center justify-center rounded-xl ${seg.bgColor}`}>
              <svg className={`h-4 w-4 ${seg.color}`} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d={seg.iconPath} />
              </svg>
            </div>
            <p className={`mt-1.5 text-[18px] font-bold tabular-nums ${seg.color}`}>
              {seg.count}
            </p>
